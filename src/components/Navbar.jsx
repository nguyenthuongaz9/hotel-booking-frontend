...
const Navbar = () => {
    ...
    return (
        <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "bg-transparent py-4 md:py-6"}`}>
            ...
            <div className="hidden md:flex items-center gap-4">
                <img src={login} alt="search" className={`${isScrolled && "invert opacity-80"} h-7 transition-all duration-500`} />
                {user ? (
                    <UserButton>
            ...
            {/* Mobile Right - Fixed: Chỉ hiển thị UserButton hoặc Menu button */}
                ...
                {user ? (
                    <>
                        ...
                        <button onClick={() => setIsMenuOpen(true)}>
                            <IoMenu className={`size-8 ${isScrolled ? "text-gray-700" : "text-white"}`} />
                        </button>
                    </>
                ) : (
                    <button onClick={() => setIsMenuOpen(true)}>
